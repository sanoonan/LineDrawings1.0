...
void Shaders :: setBackground(glm::vec3 background)
{
	glUseProgram(id);

	int background_location = glGetUniformLocation (id, "background_colour");

	glUniform3fv (background_location, 1, glm::value_ptr(background));

}

void Shaders :: setToonValues(float thresh, float shade)
{
	glUseProgram(id);

	int thresh_loc = glGetUniformLocation (id, "threshold");
	int shade_loc = glGetUniformLocation(id, "darkness");
